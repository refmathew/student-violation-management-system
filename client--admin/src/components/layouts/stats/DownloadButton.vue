  ...
  <q-page-sticky v-if="isStatPage" position="bottom-right" :offset="[32, 32]">
    <q-btn fab icon="download" color="accent" @click="handleDownload">
      <q-tooltip anchor="center left" self="center right"
        >Download violation data</q-tooltip
      >
    </q-btn>
  </q-page-sticky>
</template>
...
export default {
  ...
  methods: {
    ...
    async prepareViolationExcel(data) {
      ...
      const weekSheet = utils.json_to_sheet(
        this.transformViolationStats(data.week)
      );
      const monthSheet = utils.json_to_sheet(
        this.transformViolationStats(data.month)
      );
      const yearSheet = utils.json_to_sheet(
        this.transformViolationStats(data.year)
      );
      ...
      writeFile(workbook, "Violation_data_by_count.xlsx", {
        compression: true,
      });
    },
    async prepareCourseYearExcel(data) {
      const workbook = utils.book_new();
      const weekSheet = utils.json_to_sheet(
        this.transformCourseYearStats(data.week)
      );
      const monthSheet = utils.json_to_sheet(
        this.transformCourseYearStats(data.month)
      );
      const yearSheet = utils.json_to_sheet(
        this.transformCourseYearStats(data.year)
      );
      utils.book_append_sheet(workbook, weekSheet, "Week");
      utils.book_append_sheet(workbook, monthSheet, "Month");
      utils.book_append_sheet(workbook, yearSheet, "Year");
      writeFile(workbook, "Violation_data_by_course_and_year.xlsx", {
        compression: true,
      });
    },
    async prepareTimeExcel(data) {
      ...
      const monthSheet = utils.json_to_sheet(
        this.transformTimeStats(data.month)
      );
      const yearSheet = utils.json_to_sheet(this.transformTimeStats(data.year));
      utils.book_append_sheet(workbook, weekSheet, "Week");
      utils.book_append_sheet(workbook, monthSheet, "Month");
      utils.book_append_sheet(workbook, yearSheet, "Year");
      writeFile(workbook, "Violation_data_by_time_of_the_day.xlsx", {
        compression: true,
      });
    },
    async prepareGuardExcel(data) {
      const workbook = utils.book_new();
      const weekSheet = utils.json_to_sheet(
        this.transformGuardStats(data.week)
      );
      const monthSheet = utils.json_to_sheet(
        this.transformGuardStats(data.month)
      );
      const yearSheet = utils.json_to_sheet(
        this.transformGuardStats(data.year)
      );
      utils.book_append_sheet(workbook, weekSheet, "Week");
      utils.book_append_sheet(workbook, monthSheet, "Month");
      utils.book_append_sheet(workbook, yearSheet, "Year");
      writeFile(workbook, "Violation_data_by_guard.xlsx", {
        compression: true,
      });
    },
