  ...
  <q-page class="page--settings">
    <q-form class="page--settings__form" @submit="handleViolationSubmit">
      ...
      <div class="page--settings__form-main">
        <q-input
          class="page--settings__form-input"
        ...
        />
        <q-toggle
          class="page--settings__form-toggle"
          v-model="violation.isMajor"
          :label="violation.isMajor === 0 ? 'Minor' : 'Major'"
          :false-value="0"
          :true-value="1"
          color="deep-purple-12"
        />
...
export default {
  data() {
    return {
      ...
      inputRules: [
        (val) => (val && val.length > 0) || "Please provide details",
      ],
    };
  },
  methods: {
    async handleGuardSubmit() {
      ...
      try {
        const response = await AdminService.registerGuard(
          this.guardFirstName,
          this.guardLastName
        );
        notif({
    ...
    async handleViolationSubmit() {
      ...
      try {
        const response = await AdminService.registerViolation(
          this.violation.name,
          this.violation.number,
          this.violation.isMajor
        );
        notif({
